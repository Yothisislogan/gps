...
if [ -n "${NICANAV_BACKUP_RCLONE_TARGET:-}" ]; then
  command -v rclone >/dev/null || { echo "offsite backup is configured but rclone is missing" >&2; exit 1; }
fi
...
if ! zcat "${OUT}.tmp" | grep "CREATE TABLE public.gazetteer" >/dev/null; then
